...
class ListeningTapdTask():
            ...
            for x in self.config:
                ...
                for project in x["tapd_projects"]:
                    ...
                    if len(task_list) != 0:
                        ...
                        card_msg = self._get_not_task_message(task_list)
                        if card_msg != None:
                            # print(card_msg)
                            self._send(card_msg)
    ...
    def _get_not_task_users_template(self):
        ...
        for u in self._not_task_users_dict:
            at_users += f"<at id={self._not_task_users_dict[u]}></at>"
    ...
    def _get_not_task_message(self, task_list):
        _init_msg = {"msg_type": "interactive", "card": {"config": {"enable_forward": True}, "elements": [],
                                                         "header": {"template": "red",
        ...
        if self.now_datetime.hour >= 9 and self.now_datetime.hour <= 10:
            for u in self._feishu_user_open_ids:
                ...
                if is_exists == False:
                    self._not_task_users_dict.update({u: self._feishu_user_open_ids[u]})
        if len(self._not_task_users_dict) != 0:
